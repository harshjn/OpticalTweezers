...
#%%
phaseR = np.ravel(Rs,'F')/periodR*2*pi
phaseR = (phaseR) % (2 * np.pi)
plt.plot(phaseR[1:600000])
minX=0.7*2*pi;maxX=0.9*2*pi;
strtX=minX-(maxX-minX);
plt.hlines(minX,0,600000,'r') #Minima of potential well
plt.hlines(maxX,0,600000,'r') #Maxima of potential well
plt.hlines(strtX,0,600000,'r') #approx well start

#%%  Now we are interested in the escape time from trap. 
ResidenceTimesMat=[];

for i in range(nump):
    print(i)
    minimaX=minX;maximaX=maxX;
    startX=minX-(maxX-minX);
    rtime=0;
    while (rtime>=0):
        Rout=np.transpose(Rs[:,i,:]/periodR)[0];
        out = interp1d(dt*np.linspace(0,nums-1,nums),Rout)

        Rsmooth=np.convolve(Rout, np.ones(1000)/1000, mode='valid')
        
        reside=np.where(np.logical_and(Rsmooth>=startX/2/pi, Rsmooth<=maximaX/2/pi))
        
        if (len(reside[0])>0):
            rtime=dt*(max(reside[0])-min(reside[0]))
            ResidenceTimesMat.append(rtime)
            startX=startX+2*pi;
            maximaX=maximaX+2*pi;
        else:
            ResidenceTimesMat.pop(-1)
            rtime=-1;
            
            
print(np.mean(ResidenceTimesMat),len(ResidenceTimesMat),F0)
    #%%
    

import csv
filename='Residence'
with open(filename+"_"+str(F0)+"_"+str(dt)+"_"+str(nump)+str(nums)+".csv", "a+", newline='') as file:
    writer = csv.writer(file, delimiter='\n')
    writer.writerow(ResidenceTimesMat)
print('data written')
