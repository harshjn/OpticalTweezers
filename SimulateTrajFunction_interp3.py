...
def SimulateTraj(nums=int(1e5),dt =1e-3,nump=10,F0 = 71e-12, T=303,eta=8.9e-4):
    pi=np.pi;
    a=2e-6 # Micrometers Size of particle
    r=8e-6 #Radius of the outer circle
    periodX = 2*pi*r;
    ...
    A = np.load('uforce1e-17.npy.npz')
    d = dict(zip(("xMat","Uforce","Ufunc"), (A[k] for k in A)))
    ...
    Uforce = interp1d(xMat_,Uforce_)
    Ufunc = interp1d(xMat_,Ufunc_)
    ...
    t=Traj(dt,timeMat, Rs,T,std,periodX,Uforce_,Ufunc_,xMat,eta,zeta,F0)
